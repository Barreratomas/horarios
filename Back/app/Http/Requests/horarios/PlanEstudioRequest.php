...
class PlanEstudioRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $detalleRules = $esCreacion ? ['required', 'string', 'max:255'] : ['required', 'string', 'max:255'];
        $carreraRules = $esCreacion ? ['required', 'integer'] : ['required', 'integer'];
        $fechaInicioRules = $esCreacion  ? ['required', 'date']  : ['required', 'date'];
        $fechaFinRules = $esCreacion  ? ['nullable', 'date', 'after_or_equal:fecha_inicio']  : ['nullable', 'date', 'after_or_equal:fecha_inicio'];
